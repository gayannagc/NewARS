...
router.get('/test', function(req,res){
    var road_sign = {};
    rd_sign.find({}, (err,data) => {
        road_sign.latitude = data.lat;
        road_sign.longitude = data.long;
        road_sign.sign_name = data.sign_name;
        

        res.render('test',{
            road_signs : JSON.stringify(road_sign),
    ...
    if(!file){
        ...
        csvtojson().fromFile('./uploads/'+file.filename).then(source => {
            source.forEach((entry) => {
                ...
                let promises = [locatingFunctions.reverseGeoCoding(entry.latitude,entry.longitude),locatingFunctions.getRoad(entry.latitude,entry.longitude)];
                Promise.all(promises)
                    .then(results => {
                        let sign_data = new rd_sign();
                        if(results[1] != "error" || results != "error"){
                            sign_data.road = results[1];
                            sign_data.areaCode = results[0];
                        ...
                        req.flash('failure', 'Data was not Saved');
                        res.redirect('/roadsign/upload/csv');
                    })
            })
        })
